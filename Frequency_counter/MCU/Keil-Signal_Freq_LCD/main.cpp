...
int main() {
    ...
    T = read_T_value()*0.000001;     /* converts 16 bit wide value into decimal (seconds).
			                                 // 0.000001s = time for 1 cycle of 1MHz clock used in
			                                //  T_cycle_measurement component  */
				 
		Display_Frequency(T);                //function to calculate frequency and display value to LCD  
		
		wait(1.0);                          //request return value every second 
	}
}	
