...
--From this testbench i'm expecting to see continous sampling of the 3 ADC samples.
--i also want to see that MISO is clocked on the rising edge of SCLK & MOSI on the 
--falling edge.
     
RESET <= '1';           -- RESET active low                                            
DATATX <= "0000000000000001"; -- data to transmit to slave via MOSI line(no data needed)
                              --transmit '1' signals end for test purpose
...
MISO <= '0';   -- first four zeros from MISO read 
wait for 40 us;
end loop;

for i in 0 to 11 loop 
MISO <= '1';   -- 12bit all 1's from MISO read 
wait for 40 us;
end loop;
...
MISO <= '1';   
wait for 40 us;
MISO <= '0';   -- 12bit value '101010101010' from MISO read 
wait for 40 us;
end loop;
...
MISO <= '1';   
wait for 40 us;
MISO <= '0';   -- 12bit value '100100100100' from MISO read 
wait for 40 us;
MISO <= '0';   
wait for 40 us;
end loop;
